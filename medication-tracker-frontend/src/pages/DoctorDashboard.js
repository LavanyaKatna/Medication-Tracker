...
const DoctorDashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchHighRisk = async () => {
            try {
                const res = await api.get("/api/schedules/high-risk");
                setHighRiskPatients(res.data);
            } catch (err) {
                console.error(err);
            }
        };
            ...
            <div style={s.main}>
                {highRiskPatients.length > 0 && (
                    <div style={s.alertBanner}>
                        🚨 {highRiskPatients.length} High-Risk Patient(s) detected!
                        Medication adherence is below safe levels.
                    </div>
                )}
                <div style={s.topBar}>
                ...
                <div style={s.card}>
                    <h3 style={{ ...s.cardTitle, color: "#f87171" }}>
                        ⚠ High Risk Patients
                    </h3>

                    {highRiskPatients.length === 0 ? (
                        <div style={s.empty}>No high-risk patients</div>
                    ) : (
                        <table style={s.table}>
                            <thead>
                                <tr>
                                    <th style={s.th}>Patient ID</th>
                                    <th style={s.th}>Adherence</th>
